...
function getColor(score) {
    ...
    if (score >= 3.5) return "#2ecc71"; // Very Good
    if (score >= 3.0) return "#f1c40f"; // Good
    return "#e67e22"; // Niche
}
...
function renderMarkers() {
    ...
    // Massive, high-contrast star for Elite islands
    ...
    Object.keys(islandData).forEach(id => {
        ...
        // 4.1+ gets a Star, everything else gets a high-contrast dot
...
function showDetail(id) {
    ...
    // Switch UI and show loading state
...
function renderDetailView(d) {
    ...
    if (d.itinerary && d.itinerary.length > 0) {
        ...
        d.itinerary.forEach(stop => {
            const stopMarker = L.circleMarker([stop.lat, stop.lng], { radius: 6, fillColor: '#005BAE', color: '#fff', weight: 2, fillOpacity: 1 }).addTo(miniMap);
...
function filterIslands() {
    ...
    Object.keys(markerStore).forEach(k => {
        ...
        if (item.isStar) { 
            item.marker.setOpacity(m ? 1 : 0.1); 
        } else { 
            item.marker.setStyle({ opacity: m ? 1 : 0.1, fillOpacity: m ? 0.95 : 0.1 }); 
        }
    });
}
